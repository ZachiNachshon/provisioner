...
def install_available_plugins() -> None:
    ...
    options: List[str] = []
    hash_to_plgn_obj_dict: dict[str, ProvisionerConfig.PluginDefinition] = {}
    ...
    for package_name in packages_from_cfg:
        ...
        if package_name not in packages_from_pip_escaped:
            plgn_def = prov_cfg.plugins_definitions.get(package_name, None)
    ...
    for selected_plgn in selected_plugins:
        plgn_def = hash_to_plgn_obj_dict.get(hash(selected_plgn), None)
