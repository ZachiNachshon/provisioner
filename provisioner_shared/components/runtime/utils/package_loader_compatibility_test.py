...
class PackageLoaderCompatibilityTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.loader = PackageLoader(
            ctx=self.mock_ctx,
            io_utils=self.mock_io_utils,
            process=self.mock_process,
            pypi=self.mock_pypi_registry
        )

    @mock.patch("provisioner_shared.components.runtime.utils.package_loader.VersionCompatibility.get_package_version_from_pip")
    ...
    def test_get_runtime_version_from_version_file(self):
        """Test getting runtime version from version.txt file"""
        # Mock pip method failing
        with mock.patch("provisioner_shared.components.runtime.utils.package_loader.VersionCompatibility.get_package_version_from_pip") as mock_pip:
            mock_pip.return_value = None
            # Mock io_utils to return version from file
            self.mock_io_utils.read_file_safe_fn.return_value = "0.1.16"
