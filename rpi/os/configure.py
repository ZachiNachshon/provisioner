...
from common.sd_card.remote_os_configure import RemoteMachineConfigureCollaborators, RemoteMachineConfigureRunner, RemoteMachineConfigureArgs
...
class RPiOsConfigureArgs:
    ...
    def __init__(self, 
        ...
        ip_discovery_range: Optional[str] = None) -> None:
    ...
    def print(self) -> None:
        logger.debug(f"RPiOsConfigureArgs: \n" +
...
class RPiOsConfigureRunner:
    def run(self, ctx: Context, args: RPiOsConfigureArgs, collaborators: Collaborators) -> None:
        ...
        RemoteMachineConfigureRunner().run(
            ctx=ctx, 
            args=RemoteMachineConfigureArgs(node_username, node_password, ip_discovery_range, config.ansible_playbook_folder_path), 
