...
class PoetryManager:
    ...
    def get_package_info(self) -> PackageInfo:
        ...
        version_output = self.command_runner.run_command(
...
class GitHubReleaseManager:
    ...
    def create_release(self, tag: str, title: str, notes_file: Optional[Path] = None, 
                      is_prerelease: bool = False, target_branch: str = "master") -> None:
        """
    ...
    def download_release_assets(self, source_tag: str, download_dir: Path) -> List[Path]:
        ...
        try:
            self.command_runner.run_command(
                f'gh release download "{source_tag}" --dir "{download_dir}"'
            )
...
class PackageDeployer:
    ...
    def _check_prerequisites(self) -> None:
        ...
        if (self.args.command == "upload" 
            and self.args.upload_action == "upload-to-pypi"):
    ...
    def _handle_build_command(self) -> None:
        ...
        # Build the package
        self.built_package_path = self.poetry_manager.build_package(
            self.args.build_type, 
            self.args.multi_project
        )
    ...
    def _handle_compression_if_needed(self) -> Path:
        ...
        # Compress the file
        compressed_path = FileManager.compress_file_to_tar_gz(
            self.built_package_path,
            output_dir,
            compressed_name
        )
...
class PackageDeployerCLI:
    ...
    def _parse_arguments(self) -> CliArguments:
        ...
        build_parser.add_argument('--build-type', required=True, choices=['sdist', 'wheel'],
                                help='Package build type [options: sdist/wheel]')
        build_parser.add_argument('--version', 
                                help='Custom version to build (default: current poetry version)')
        build_parser.add_argument('--compress', choices=['tar.gz'],
                                help='Release asset format [options: tar.gz]')
        ...
        upload_parser.add_argument('--upload-action', required=True, 
                                 choices=['promote-rc', 'upload-to-pypi'],
                                 help='Upload action [options: promote-rc/upload-to-pypi]')
        upload_parser.add_argument('--source-tag', required=True,
                                 help='Source GitHub release tag to download (example: v1.0.0-RC.1)')
        upload_parser.add_argument('--release-tag',
                                 help='Target release tag for promote-rc action (example: v1.0.0)')
        upload_parser.add_argument('--release-title',
        ...
        prerelease_parser.add_argument('--release-tag', required=True,
                                     help='GitHub release tag (example: v1.0.0-RC.1)')
        prerelease_parser.add_argument('--release-title',
                                     help='GitHub release title')
        prerelease_parser.add_argument('--release-notes-file',
                                     help='Path to release notes file')
        prerelease_parser.add_argument('--assets-dir',
