...
class TestFileManager(unittest.TestCase):
    ...
    def test_compress_file_to_tar_gz(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Compress file
            compressed_file = FileManager.compress_file_to_tar_gz(test_file, output_dir, "compressed.tar.gz")
    ...
    def test_compress_file_not_found(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with self.assertRaises(FileNotFoundError):
                FileManager.compress_file_to_tar_gz(non_existent, output_dir, "test.tar.gz")
...
class TestGitHubReleaseManager(unittest.TestCase):
    ...
    def test_create_release_success(self, mock_run):
        ...
        with patch.object(self.github_manager, "check_release_exists", return_value=False):
            self.github_manager.create_release("v1.0.0", "Test Release", is_prerelease=True, target_branch="develop")
...
class TestPackageDeployer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.env_patcher = patch.dict(os.environ, {"GITHUB_TOKEN": "test-token", "PYPI_API_TOKEN": "test-token"})
...
class TestPackageDeployerCLI(unittest.TestCase):
    ...
    @patch(
        "sys.argv", ["package_deployer.py", "upload", "--upload-action", "promote-rc", "--source-tag", "v1.0.0-RC.1"]
    )
    ...
    @patch("sys.argv", ["package_deployer.py", "upload", "--upload-action", "upload-to-pypi", "--source-tag", "v1.0.0"])
...
class TestIntegrationScenarios(unittest.TestCase):
    ...
    def test_build_workflow_with_compression(self, mock_build, mock_get_package, mock_update_lock, mock_check_tool):
        ...
        # Setup arguments
        args = CliArguments(
            command="build", build_type="wheel", compress="tar.gz", output_path=str(self.temp_dir / "output")
        )
    ...
    def test_promote_rc_workflow(
        self,
        mock_upload_assets,
        mock_create_release,
        mock_download_assets,
        mock_get_package,
        mock_update_lock,
        mock_check_tool,
    ):
    ...
    def test_prerelease_workflow(
        self, mock_upload_from_dir, mock_create_release, mock_get_package, mock_update_lock, mock_check_tool
    ):
    ...
    def test_upload_ga_to_pypi_workflow(
        self,
        mock_upload_package,
        mock_extract,
        mock_download_assets,
        ...
        # Setup arguments
        args = CliArguments(command="upload", upload_action="upload-to-pypi", source_tag="v1.0.0")
