...
class PostReleaseVersionBumper:
    ...
    def _run_command(self, command: list, cwd: Optional[Path] = None, check: bool = True) -> subprocess.CompletedProcess:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Bump project versions for next development cycle after GA release"
    )
    parser.add_argument(
        "--project-name",
        required=True,
        help="Name of the project that was released (e.g., 'provisioner')"
    )
    parser.add_argument(
        "--stable-version",
        required=True,
        help="The stable version that was just released (e.g., '1.2.3')"
    )
